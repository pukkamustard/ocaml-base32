...
let encode_sub pad {emap; _} ?(off = 0) ?len input =
  ...
  then error_msgf "Invalid bounds"
    ...
    let emit b1 b2 b3 b4 b5 i =
      unsafe_set_uint8 res i
        (emap ((0b11111000 land b1) lsr 3));
      unsafe_set_uint8 res (i + 1)
        (emap (((0b00000111 land b1) lsl 2)
               lor ((0b11000000 land b2) lsr 6)));
      unsafe_set_uint8 res (i + 2)
        (emap ((0b00111110 land b2) lsr 1));
      unsafe_set_uint8 res (i + 3)
        (emap (((0b00000001 land b2) lsl 4)
               lor ((0b11110000 land b3) lsr 4)));
      unsafe_set_uint8 res (i + 4)
        (emap (((0b00001111 land b3) lsl 1)
               lor ((0b10000000 land b4) lsr 7)));
      unsafe_set_uint8 res (i + 5)
        (emap ((0b01111100 land b4) lsr 2));
      unsafe_set_uint8 res (i + 6)
        (emap (((0b00000011 land b4) lsl 3)
               lor ((0b11100000 land b5) lsr 5)));
      unsafe_set_uint8 res (i + 7)
        (emap (0b00011111 land b5)) in

    let rec enc j i =
      if i = len then
        ()
      else if i = n - 1 then
        emit (unsafe_get_uint8 input (off + i)) 0 0 0 0 j
      else if i = n - 2 then
        emit (unsafe_get_uint8 input (off + i))
        ...
        (emit (unsafe_get_uint8 input (off + i))
           ...
           (unsafe_get_uint8 input (off + i + 2))
           (unsafe_get_uint8 input (off + i + 3))
           (unsafe_get_uint8 input (off + i + 4)) j;
         enc (j + 8) (i + 5))
    in
    ...
    if pad then (
      ...
      Ok (Bytes.unsafe_to_string res, 0, n' - pad_to_write)
